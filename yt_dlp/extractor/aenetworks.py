...
class AENetworksBaseIE(ThePlatformIE):  # XXX: Do not subclass from concrete IE
    ...
    # Third element is the environment variable name holding the Adobe Pass
    # software statement for that network, or None if Adobe Pass is not used.
    # Required env vars: AENETWORKS_HISTORY_SOFTWARE_STATEMENT,
    # AENETWORKS_AETV_SOFTWARE_STATEMENT, AENETWORKS_LIFETIME_SOFTWARE_STATEMENT,
    # AENETWORKS_FYI_SOFTWARE_STATEMENT
    _DOMAIN_MAP = {
        'history.com': ('HISTORY', 'history', 'AENETWORKS_HISTORY_SOFTWARE_STATEMENT'),
        'aetv.com': ('AETV', 'aetv', 'AENETWORKS_AETV_SOFTWARE_STATEMENT'),
        'mylifetime.com': ('LIFETIME', 'lifetime', 'AENETWORKS_LIFETIME_SOFTWARE_STATEMENT'),
        'fyi.tv': ('FYI', 'fyi', 'AENETWORKS_FYI_SOFTWARE_STATEMENT'),
    ...
    def _extract_aetn_info(self, domain, filter_key, filter_value, url):
        requestor_id, brand, software_statement_env = self._DOMAIN_MAP[domain]
        ...
        if theplatform_metadata.get('AETN$isBehindWall'):
            if software_statement_env:
                software_statement = os.environ.get(software_statement_env)
                if not software_statement:
                    raise ExtractorError(
                        f'This content requires Adobe Pass authentication. '
                        f'Set the {software_statement_env} environment variable.',
                        expected=True)
            else:
